...
The shipped Logstash configuration allows you to send data over the TCP port 5000. For example, you can use one of the
...
cat /path/to/logfile.log | nc -q0 localhost 5000          # BSD
cat /path/to/logfile.log | nc -c localhost 5000           # GNU
cat /path/to/logfile.log | nc --send-only localhost 5000  # nmap
```
...
The Kibana configuration is stored in [`elasticsearch-config/export.ndjson`][config-es]. This needs to be imported into Kibana once it is up under Management -> Saved Objects
